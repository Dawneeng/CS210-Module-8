...
// Display a multiplication table; asks user for a number.
void displayMultiplicationTable() {
	cout << "Enter a number \n";
	int number;
	cin >> number;
	int result = callIntFunc("MultiplicationTable", number);

}
// Display a doubled value; ask user for a number.
void doubleAValue() {
	cout << "Enter a number to be doubled\n";
	int number;
	cin >> number;
	int result = callIntFunc("DoubleValue", number);
	cout << result << endl;
...
int main()
{
	...
	do {
		...
		if (userChoice == 1) {
			...
			cout << "Enter valid choice, 1, 2 or 3." << endl;
		}
